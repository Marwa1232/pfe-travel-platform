...
const AdminDashboard: React.FC = () => {
  ...
  const [newDestinationImageFile, setNewDestinationImageFile] = useState<File | null>(null);
  ...
  const handleCreateDestination = async () => {
    try {
      ...
      if (newDestinationImageFile) {
        ...
        formData.append('country', newDestination.country);
        formData.append('region', newDestination.region || '');
        formData.append('image', newDestinationImageFile);
  ...
  const handleUpdateDestination = async () => {
    try {
      ...
      if (selectedImageFile) {
        ...
        formData.append('country', editingDestination.country);
        formData.append('region', editingDestination.region || '');
        ...
        console.log('FormData contents:', {
          ...
          region: editingDestination.region,
          image: selectedImageFile
        });
        
        await adminAPI.updateDestination(editingDestination.id, formData);
      } else {
        // Use regular JSON if no file
        console.log('Sending JSON without file:', editingDestination);
        await adminAPI.updateDestination(editingDestination.id, editingDestination);
      }
      ...
      <Container maxWidth="xl" sx={{ py: 3 }}>
                          ...
                          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                            ...
                            <Box sx={{ display: 'flex', gap: 0.5 }}>
                              ...
                              <Tooltip title="View Details">
                                <IconButton size="small" onClick={() => navigate(`/admin/organizers/${organizer.id}`)}>
              ...
              <Stack spacing={2}>
                ...
                <Box sx={{ mt: 1, mb: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: 0.5 }}>
                    ...
                    >
                      {newDestinationImageFile ? newDestinationImageFile.name : 'Télécharger'}
                      <input 
                        type="file" 
                        hidden 
                        accept="image/*" 
                        onChange={(e) => {
                          ...
                          if (file) setNewDestinationImageFile(file);
                        }} 
                      />
                    </Button>
                    {newDestinationImageFile && (
        ...
        >
          <DialogTitle sx={{
            ...
            {editType === 'category' ? 'Modifier la catégorie' : 'Modifier la destination'}
          </DialogTitle>
              ...
              <Stack spacing={2} sx={{ mt: 1 }}>
                ...
                <Box sx={{ mt: 1, mb: 1 }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mt: 0.5 }}>
                    ...
                    >
                      {selectedImageFile ? selectedImageFile.name : 'Télécharger'}
                      <input 
                        type="file" 
                        hidden 
                        accept="image/*" 
          ...
          <DialogActions sx={{ p: 2, pt: 0 }}>
            <Button size="small" onClick={() => setEditDialogOpen(false)} sx={{ color: '#666' }}>
              Annuler
            </Button>
            <Button
              size="small"
              variant="contained"
              onClick={editType === 'category' ? handleUpdateCategory : handleUpdateDestination}
              sx={{
                backgroundColor: '#00BFA5',
                '&:hover': { backgroundColor: '#0D47A1' },
                borderRadius: 2,
