...
const TripList: React.FC = () => {
  ...
  useEffect(() => {
    loadTrips();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filters, page, sortBy, selectedDestinations, selectedCategories]);

  const loadInitialData = async () => {
    try {
      const destinationsRes = await destinationAPI.popular(); // Utilisation de popular() au lieu de getAll()
  ...
  const loadTrips = async () => {
    try {
      ...
      setTrips(response.data['hydra:member'] || response.data);
      setTotalPages(Math.ceil((response.data['hydra:totalItems'] || response.data.length) / 12));
    } catch (error) {
