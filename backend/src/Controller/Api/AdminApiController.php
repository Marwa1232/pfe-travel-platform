...
class AdminApiController extends AbstractController
{
    ...
    #[Route('/categories/{id}', name: 'api_admin_category_detail', methods: ['GET', 'PUT', 'DELETE'])]
    public function manageCategory(Request $request, int $id): JsonResponse
    {
        $authCheck = $this->requireAdmin($request);
        if ($authCheck instanceof JsonResponse) {
            return $authCheck;
    ...
    #[Route('/destinations/{id}', name: 'api_admin_destination_detail', methods: ['GET', 'PUT', 'DELETE'])]
    public function manageDestination(Request $request, int $id): JsonResponse
    {
        $authCheck = $this->requireAdmin($request);
        if ($authCheck instanceof JsonResponse) {
            return $authCheck;
        ...
        if ($request->isMethod('DELETE')) {
            ...
            $this->em->flush();
            return $this->json(['message' => 'Destination deleted']);
        }

        if ($request->isMethod('PUT')) {
            // Debug: check what's received
            $debug = [
                'hasFiles' => $request->files->count(),
            ...
            if ($request->files->count() > 0) {
                ...
                if ($file) {
                    $uploadDir = dirname(__DIR__, 3) . '/public/uploads/destinations';
                    if (!is_dir($uploadDir)) {
                    ...
                    if (file_put_contents($destinationPath, file_get_contents($file->getPathname())) !== false) {
                        $imagePath = '/uploads/destinations/' . $newFilename;
                    }
                }
            }
            
            // Handle form data - for FormData, text fields are in $request->request
            $data = [];
            ...
            // Merge with form request data if exists (for FormData submissions)
            ...
            if ($imagePath) {
                $destination->setImage($imagePath);
    ...
    public function manageDestinations(Request $request): JsonResponse
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            return $this->json(['message' => 'Destination created', 'id' => $destination->getId(), 'image' => $destination->getImage()], Response::HTTP_CREATED);
        }
